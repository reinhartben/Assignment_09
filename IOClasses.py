...
# BReinhart, 2021-Mar-09, Added Track file writing functionality
# BReinhart, 2021-Mar-09, Added Track file loading functionality
#------------------------------------------#
        ...
        try:
            ...
            with open(file_name_track, 'w') as file:
                for disc in lst_Inventory:
                    ...
                    cd_id = disc.cd_id
                    for song in tracks:
                        file.write('{},{}'.format(cd_id, song.get_record()))
        except Exception as e:
            print('There was a general error Saving!', e, e.__doc__, type(e), sep='\n')
    ...
    def load_inventory(file_name: list) -> list:
        ...
        try:
            ...
            with open(file_name_track, 'r') as file:
                for line in file:
                    data = line.strip().split(',')
                    cd_idx = PC.DataProcessor.select_cd(lst_Inventory, int(data[0]))
                    song = DC.Track(int(data[1]), data[2], data[3])
                    cd_idx.add_track(song)
        except Exception as e:
            print('There was a general error Loading!', e, e.__doc__, type(e), sep='\n')
